...
<div style="margin-top:2em;">
	<form class="row g-3" method="get" action="ReportePrestamos">
		<div class="col-md-3">
		    ...
		    <input type="text" name="desde"  class="form-control" id="datepickerdesde" >
		  </div>
		  <div style="margin-left:15px;" class="col-md-3">
		    <label for="datepickerhasta" class="form-label">Hasta</label>
		    <input type="text" name="hasta" class="form-control" id="datepickerhasta" >
		  </div>
	...
	<table class="table table-striped table-sm">
		<tr> <th>Numero de Cuenta</th>  <th>Tipo de cuenta</th>  <th>Fecha de Alta de Cuenta</th> <th> </th> </tr>
		...
		%>
		<tr>  
			<form name="formulario" action="ReportePrestamos?idCuenta=<%=cuenta.getN_Cuenta()%>" method="get">
				<td><%=cuenta.getN_Cuenta() %>    <input type="hidden" name="idCuenta" value="<%=cuenta.getN_Cuenta()%>">
				<td><%=cuenta.getTipoCuenta().getDescripcion() %></td>   
				<td><%=cuenta.getFecha() %></td>
				<td> <input type="submit" class="btn btn-info" name="btnMostrar" value="Mostrar Detalles"> </td>  
			</form> 
		</tr>
		<%  } %>
	</table>
